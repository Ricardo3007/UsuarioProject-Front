...
import { ResponseStructure } from './models/response-api.model';
/**
 *
 */
@Injectable({
...
export class AppService {
  ...
  sentRequest$<T>(requestStructure: RequestStructure): Observable<ResponseStructure<T>> {
    ...
    let response$: Observable<ResponseStructure<T>>

          const apiUrl = environment.api_url+ requestStructure.endpoint;

          switch (requestStructure.request.type) {
            case TypeRequest.DELETE:
              response$= http.delete<ResponseStructure<T>>(apiUrl, {
                headers: this._headers,
                params: this._params
              });
              break;
            case TypeRequest.POST:
              response$= http.post<ResponseStructure<T>>(apiUrl, (requestStructure.request as PostRequest).body, {
                headers: this._headers,
                params: this._params
              });
              break;
            case TypeRequest.PUT:
              response$=  http.put<ResponseStructure<T>>(apiUrl, (requestStructure.request as PutRequest).body, {
                headers: this._headers,
                params: this._params
              });
              break;
            default:
              response$=  http.get<ResponseStructure<T>>(apiUrl, {
                headers: this._headers,
                params: this._params
              });
          }
  ...
  private processResponse$<T>(
    response$: Observable<ResponseStructure<T>>,
    statusMessages?: StatusMessagesRequest
  ): Observable<ResponseStructure<T>> {
    return response$.pipe(
      map((resp) => {
        //despues hay que hacer algo con los notificacion de prime
      ...
      catchError((error: unknown) => {
        //despues hay que hacer algo con los notificacion de prime
        /*  this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Ocurrio un error', life: 2000 }); */
